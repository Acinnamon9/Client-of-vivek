...
const Timeline: React.FC = () => {
  return (
    ...
    >
      <Container>
        <div className="bg-(--card) rounded-[40px] p-12 sm:p-20 relative overflow-hidden shadow-2xl border border-(--border)">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,rgba(255,87,34,0.05),transparent_60%)]"></div>
          <h3 className="text-3xl sm:text-4xl font-black text-(--foreground) text-center mb-16 relative z-10 uppercase tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
            {timelineEvents.map((item, idx) => (
              <div key={idx} className="text-center group">
                ...
                <div className="text-xl font-black text-(--foreground) mb-3 uppercase">
                  {item.event}
                </div>
                <div
                  className={cn(
