...
const AuditTable: React.FC = () => {
  ...
  return (
    <div className="relative animate-fade-in-up delay-200 w-full max-w-2xl lg:ml-auto">
      <div className="bg-(--card) border border-(--border) rounded-[32px] p-8 md:p-10 shadow-2xl relative overflow-hidden ring-1 ring-(--border) group/card hover:ring-brand-primary/20 transition-all duration-700">
        {/* Header */}
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isLast
                      ? "bg-(--muted) border border-(--border) mt-6 shadow-2xl ring-1 ring-(--border)"
                      : "hover:bg-(--muted)/50",
                    isHovered &&
                      !isLast &&
                      "scale-[1.02] bg-(--muted) transition-transform",
                  )}
                >
                  {/* Hover Glow Effect */}
                  {isHovered && (
                    <div className="absolute inset-0 bg-linear-to-r from-transparent via-(--foreground)/5 to-transparent skew-x-12 animate-shimmer pointer-events-none"></div>
                  )}
